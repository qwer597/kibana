...
import {
  ALERT_DETAILS_FLYOUT_TABLE_TAB_ID_ROW,
  ALERT_DETAILS_FLYOUT_TABLE_TAB_ROW_CELL_COPY_TO_CLIPBOARD,
  ALERT_DETAILS_FLYOUT_TABLE_TAB_TIMESTAMP_ROW,
...
describe.skip('Alert details expandable flyout right panel', { testIsolation: false }, () => {
  ...
  it('should display and filter the table', () => {
    cy.get(ALERT_DETAILS_FLYOUT_TABLE_TAB_TIMESTAMP_ROW).should('be.visible');
    cy.get(ALERT_DETAILS_FLYOUT_TABLE_TAB_ID_ROW).should('be.visible');
  ...
  it('should test copy to clipboard cell actions', () => {
    copyToClipboardTableTabTable();
    cy.get(ALERT_DETAILS_FLYOUT_TABLE_TAB_ROW_CELL_COPY_TO_CLIPBOARD).should('be.visible');
  });
});
